...
@section('contenidoPrincipal')
    <div class="forms bg-light border border-4 rounded-3">
        <h5 class="card-header text-white bg-secondary fw-bold">EDITAR CATEGORÍA</h5>
        <div class="card-body">
            <form method="post" action="{{ route('categoria_actualizar', $category->id) }}" class="row g-3"
                ...
                <div class="col-md-6 form-group">
                    ...
                    <input type="hidden" id="current_image" name="current_image"
                        value="{{ stream_get_contents($category->image) }}" />
                </div>
                <div class="mb-2 form-group text-center">
                    <button type="reset" class="btn resetbutton mt-3">BORRAR CAMPOS</button>
                    <button type="submit" id="submit" class="btn btn-primary mt-3">EDITAR CATEGORÍA</button>
            </form>
        </div>
    </div>
    <script>
    ...
    <script src="{{ asset('js/categorias/editar_categoria.js') }}"></script>
@endsection
